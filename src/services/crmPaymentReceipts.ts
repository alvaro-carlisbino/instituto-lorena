...
export async function fetchReceiptsForPayments(paymentIds: string[]): Promise<PaymentReceiptRow[]> {
  ...
  const { data, error } = await supabase
    ...
    .select('id, payment_id, storage_path, file_name, mime_type, file_size, note, created_at')
...
export async function uploadPaymentReceipt(paymentId: string, file: File, note?: string): Promise<PaymentReceiptRow> {
...
export async function deletePaymentReceipt(id: string, storagePath: string): Promise<void> {
  if (!supabase) throw new Error('Sistema não configurado.')
