...
export async function listPostConsultation(): Promise<PostConsultationLead[]> {
  const client = assertClient()
  const [leadsRes, dispensadosRes] = await Promise.all([
    client
      .from('leads')
      ...
      .limit(300),
    client.from('post_consultation_dismissals').select('lead_id').limit(1000),
  ])
  if (leadsRes.error) throw new Error(leadsRes.error.message)
  if (dispensadosRes.error) throw new Error(dispensadosRes.error.message)
  const dispensados = new Set(
    (dispensadosRes.data ?? []).map((r) => String((r as Record<string, unknown>).lead_id)),
  )
  const data = (leadsRes.data ?? []).filter(
    (r) => !dispensados.has(String((r as Record<string, unknown>).id)),
  )
  const hoje = Date.now()
  return (data ?? []).map((r) => {
    const row = r as Record<string, unknown>
...
/**
 * Tira paciente da fila de pós-consulta sem dar destino a ele.
 *
 * Serve para o caso em que encaminhar seria pior que não fazer nada: backlog de
 * meses atrás, e contato que nunca foi paciente (a etapa recebeu "Kumon" e
 * "LRM Empreendimentos Imobiliários" por roteamento). O lead continua no funil,
 * na mesma etapa, com o histórico inteiro — o que muda é que a fila para de
 * cobrar ação. Reversível: `reabrirNaFila` devolve.
 */
export async function dispensarDaFila(leadIds: string[], reason: string): Promise<number> {
  const client = assertClient()
  if (leadIds.length === 0) return 0
  const { data, error } = await client
    .from('post_consultation_dismissals')
    .upsert(
      leadIds.map((leadId) => ({ lead_id: leadId, reason: reason.trim() || null })),
      { onConflict: 'lead_id' },
    )
    .select('lead_id')
  if (error) throw new Error(error.message)
...
export async function reabrirNaFila(leadId: string): Promise<void> {
  const client = assertClient()
  const { error } = await client.from('post_consultation_dismissals').delete().eq('lead_id', leadId)
  if (error) throw new Error(error.message)
}

export type DispensadoRow = {
  leadId: string
  patientName: string
  reason: string | null
  dismissedAt: string
...
export async function listDispensados(): Promise<DispensadoRow[]> {
  const client = assertClient()
  const { data, error } = await client
    .from('post_consultation_dismissals')
    .select('lead_id, reason, dismissed_at, leads(patient_name)')
    .order('dismissed_at', { ascending: false })
    .limit(300)
  if (error) throw new Error(error.message)
  return (data ?? []).map((r) => {
    const row = r as Record<string, unknown>
    const lead = row.leads as { patient_name?: unknown } | null
    return {
      leadId: String(row.lead_id),
