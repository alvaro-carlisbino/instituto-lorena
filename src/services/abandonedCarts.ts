...
export type AbandonedCart = {
  ...
  gclid: string | null
  alreadyCustomer: boolean // telefone já aparece numa compra paga (comprou por outro caminho)
}
...
/** Telefones (chave de 8 dígitos) que já têm compra paga — pra marcar quem comprou por fora. */
async function fetchBuyerPhoneKeys(): Promise<Set<string>> {
  const set = new Set<string>()
  if (!supabase) return set
  const since = new Date(Date.now() - 60 * 24 * 3600 * 1000).toISOString()
  const [rede, asaas] = await Promise.all([
    supabase.from('rede_payments').select('phone').eq('status', 'paid').gte('created_at', since).limit(5000),
    supabase.from('asaas_payments').select('phone').in('status', ['paid', 'received', 'confirmed']).gte('created_at', since).limit(5000),
  ])
  for (const r of (rede.data ?? []) as Array<{ phone: unknown }>) { const k = phoneKey(r.phone); if (k) set.add(k) }
  for (const r of (asaas.data ?? []) as Array<{ phone: unknown }>) { const k = phoneKey(r.phone); if (k) set.add(k) }
  return set
}

export async function fetchAbandonedCarts(days = 30): Promise<AbandonedCartsResult> {
  if (!supabase) return { carts: [], anonymousCount: 0, recoverableValueCents: 0 }

  const start = new Date(Date.now() - days * 24 * 3600 * 1000).toISOString()
  const [eventsRes, buyerKeys] = await Promise.all([
    supabase
      ...
      .limit(ROW_LIMIT),
    fetchBuyerPhoneKeys().catch(() => new Set<string>()),
  ])
  ...
  for (const a of bySession.values()) {
    ...
    carts.push({
      ...
      source,
      gclid,
      alreadyCustomer: buyerKeys.has(phoneKey(phone)),
    })
  }

  carts.sort((x, y) => y.lastSeen.localeCompare(x.lastSeen))
  const recoverableValueCents = carts.filter((c) => !c.alreadyCustomer).reduce((s, c) => s + c.valueCents, 0)
  return { carts, anonymousCount, recoverableValueCents }
}
