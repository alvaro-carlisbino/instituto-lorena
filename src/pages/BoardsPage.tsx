  ...
  return (
    <AppLayout title="Boards e pipelines" subtitle="Configure funis, etapas e regras de movimentação do kanban.">
      <div className="flex flex-wrap gap-2 mb-8">
        <Button type="button" onClick={crm.addPipeline} className="rounded-none uppercase tracking-widest font-bold">
          Novo pipeline
        </Button>
      </div>
          ...
          <Card key={pipeline.id} className="shadow-none rounded-none border border-border bg-card">
            <CardHeader className="flex flex-col gap-3 space-y-0 sm:flex-row sm:items-center sm:justify-between border-b border-border/50 bg-muted/10 pb-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" size="sm" className="rounded-none uppercase tracking-widest font-semibold" onClick={() => crm.addStageToPipeline(pipeline.id)}>
                  Nova etapa
                </Button>
                <Button type="button" variant="destructive" size="sm" className="rounded-none uppercase tracking-widest font-semibold" onClick={() => crm.removePipeline(pipeline.id)}>
                  Remover
                </Button>
              </div>
            </CardHeader>
