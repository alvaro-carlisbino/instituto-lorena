...
- Opcional — ManyChat: com **`MANYCHAT_API_KEY`**, o webhook pode usar modo **queued** (resposta rápida + IA/push em background). Sem a key, resposta é **síncrona** com `reply` no JSON. `MANYCHAT_ASYNC_ACK=false` força sempre síncrono
