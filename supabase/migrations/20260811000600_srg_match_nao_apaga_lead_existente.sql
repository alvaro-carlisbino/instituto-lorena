-- Achado ao testar a blindagem da 000500: rodando srg_match_patients(false), o numero de
-- cirurgias com lead CAIU de 66 para 63 na faixa 'auto'.
--
-- Causa: o casamento fazia
--     lead_id = case when g.n = 1 then g.lead_id else s.lead_id end
-- e g.lead_id vem de shosp_patients, onde 500 dos 698 pacientes NAO tem lead. Ou seja:
-- casar o prontuario com sucesso APAGAVA o lead que tinha vindo de outra rota (da venda, ou
-- de vinculo manual). Silencioso, e a cada 2 horas, desde que o espelho existe.
--
-- coalesce: o casamento por nome pode ACRESCENTAR lead, nunca tirar. Mesma coisa para o
-- proprio prontuario.
--
-- Prova: depois do fix, srg_match_patients(false) roda duas vezes seguidas e o total de
-- cirurgias com lead fica parado em 72.
...
begin
  ...
  upd as (
    update public.srg_surgeries s
       set shosp_prontuario = case when g.n = 1 then coalesce(g.pront, s.shosp_prontuario) else s.shosp_prontuario end,
           lead_id          = case when g.n = 1 then coalesce(g.lead_id, s.lead_id)        else s.lead_id end,
