-- Fase 2 Shosp: RPC de analytics v2 — funil real cruzando leads x agendamentos
-- Shosp (agendado->comparecido->no-show->cancelado), conversão por origem, por
-- atendente, motivos de perda e tempo por etapa (gargalos). Filtro de período
-- custom + origem + responsável. SECURITY DEFINER (lê leads + shosp_* via RPC).
...
begin
  ...
  create temp table _lead_appt on commit drop as
  select a.lead_id,
         bool_or(a.status ilike 'agendad%' or a.status ilike 'confirmad%') as agendado,
         bool_or(a.status ilike 'atendid%' or a.status ilike 'comparec%' or a.status ilike 'realizad%') as comparecido,
  ...
  select coalesce(jsonb_agg(x), '[]'::jsonb) into v_by_source from (
    select jsonb_build_object(
      ...
      'total', count(*),
      'agendados', count(*) filter (where la.agendado),
      'comparecidos', count(*) filter (where la.comparecido),
      'perdidos', count(*) filter (where l.lost_reason is not null),
      'conversao_pct', round(100.0 * count(*) filter (where la.agendado) / nullif(count(*),0), 1)
    ) as x
    from _leads l left join _lead_appt la on la.lead_id = l.id
  ...
  select coalesce(jsonb_agg(x), '[]'::jsonb) into v_by_sdr from (
    ...
    from _leads l
    left join _lead_appt la on la.lead_id = l.id
