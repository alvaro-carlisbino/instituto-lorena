-- Correções no funil real (crm_analytics_v2):
--   #2 (período): o "Agendados/Compareceram" do funil passa a contar pela DATA do
--      agendamento (a partir do início do período, INCLUINDO consultas futuras —
--      "agendada" numa clínica é justamente a consulta marcada pra frente), e não
--      mais exige que o lead tenha sido CRIADO no período (um lead antigo que marcou
--      consulta agora antes era ignorado). by_source/by_sdr usam a mesma base.
--   #3 (comparecimento): a Shosp NÃO registra comparecimento (só Agendado/
--      Confirmado/Desmarcado/Faltou). "Compareceram" passa a ser um PROXY:
--      consulta com data passada que não foi cancelada/faltou = compareceu.
-- Métricas de volume/etapa/perda (total_leads, by_stage, lost_reasons,
-- time_in_stage) seguem como coorte de leads CRIADOS no período (inalteradas).
...
begin
  ...
  -- Agendamentos Shosp por lead com a DATA da consulta dentro do período (#2).
  -- comparecido é PROXY (#3): consulta passada não cancelada/faltou = compareceu.
  ...
  -- Funil headline: TODOS os leads (qualquer data de criação) com consulta no
  -- período, respeitando os filtros de origem/responsável (#2).
  select jsonb_build_object(
