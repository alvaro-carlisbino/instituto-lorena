-- A busca global (⌘K) passa a achar CLIENTE, não só paciente.
--
-- O ⌘K nasceu para o lado da clínica: procurava por nome, telefone do lead, CPF do
-- Shosp e nome da tricoscopia. Quem compra Tricopill entrava só por nome e telefone —
-- e é justamente quem liga dizendo o CPF ("é o do meu pedido"), o e-mail do checkout
-- ou o número do pedido do Bling. Nenhum desses caía aqui: 218 CPFs em
-- `custom_fields->cadastro`, 142 em `rede_payments.customer_doc`, 142 e-mails, 100
-- pedidos do Bling e 53 NF-e estavam no banco e fora da busca.
--
-- O caso que motivou: "Rosana Longuinho Regilio de Souza" no pagamento, "Rosana
-- Regilio" no card do lead. Quem digitava o nome do cartão não achava ninguém.
--
-- O escopo de polo (`current_tenant_id()`) fica como estava: esta migração amplia o QUE
-- se acha, não QUEM pode ver.
create or replace function public.crm_buscar_pacientes(p_termo text, p_limit integer default 20)
returns table(
...
as $function$
  ...
  por_lead as (
    ...
    select distinct on (lid) lid, motivo, peso from (
      ...
      where rp.tenant_id = public.current_tenant_id() and rp.lead_id is not null
      ...
      where pc.tenant_id = public.current_tenant_id() and pc.lead_id is not null
        and length(t.digitos) >= 6
      ...
      where s.tenant_id = public.current_tenant_id() and s.lead_id is not null
        and ((length(t.digitos) >= 6
  ...
  select r.tipo, r.ref, r.lead_id, r.nome, r.telefone, r.prontuario, r.cpf, r.achado_por,
         r.consultas, r.cirurgias, r.exames, r.vendas, r.ultimo_contato
  from (
