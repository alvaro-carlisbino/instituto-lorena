...
/**
 * O que estes testes protegem: a promoção de agosto/2026 (kit 3+1 com frete grátis) morava
 * num env setado na mão, e env não conhece data. Em 01/09 a IA pararia de anunciar — o texto
 * do prompt e o gancho do reengajamento têm trava de data — e o servidor continuaria zerando
 * o frete do 3+1 calado, dando frete de graça sem ninguém ver.
 *
 * A virada é pela data LOCAL de São Paulo, não pela UTC: às 23h de 31/08 em Maringá o relógio
 * UTC já está em 01/09, e a promo ainda tem que valer.
 */
...
Deno.test('durante a promo, 3+1 e 5 meses têm frete grátis', () => {
  const kits = freeShippingKits(new Date('2026-08-19T15:00:00Z'))
...
Deno.test('depois da promo, só o kit de 5 meses continua grátis', () => {
  const kits = freeShippingKits(new Date('2026-09-01T15:00:00Z'))
  assertEquals(kits.has('5_meses'), true)
  assertEquals(kits.has('3_meses'), false)
})

Deno.test('a virada é pela data de São Paulo, não pela UTC', () => {
  // 01/09 02:00 UTC = 31/08 23:00 em São Paulo: a promo AINDA vale.
  assertEquals(freeShippingKits(new Date('2026-09-01T02:00:00Z')).has('3_meses'), true)
  // 02/09 02:00 UTC = 01/09 23:00 em São Paulo: acabou.
  assertEquals(freeShippingKits(new Date('2026-09-02T02:00:00Z')).has('3_meses'), false)
})

Deno.test('o último dia da promo é o 31/08/2026', () => {
  assertEquals(PROMO_FRETE_KIT3_ATE, '2026-08-31')
})

Deno.test('sem env, isFreeShippingKit segue a data', () => {
  semEnv(() => {
    const durante = new Date('2026-08-19T15:00:00Z')
    const depois = new Date('2026-09-01T15:00:00Z')
...
Deno.test('o env continua sendo override manual e desliga a regra de data', () => {
  ...
  try {
    ...
    assertEquals(isFreeShippingKit('5_meses', new Date('2026-08-19T15:00:00Z')), false)
  } finally {
