  ...
  try {
    ...
    if (isInternal && context.leadId && actionChunks.length > 0 && reply.trim()) {
      ...
      if (redeLink?.detail && !reply.includes(redeLink.detail)) {
        ...
        // Parcelamento REAL do kit (servidor já limita): 1x = sem texto de parcela; >1 = "em até Nx sem juros".
        const inst = Number(redeLink.installments) || 0
        const parcela = inst > 1 ? ` (em até ${inst}x sem juros)` : ''
