...
Deno.serve(async (req) => {
  try {
    ...
    const SALES_MODE_BLOCK = [
      ...
      'FRETE NO LINK: PREFIRA o caminho robusto — em vez de "freight_cents", passe SEMPRE "freight_service" com o serviço escolhido ("PAC" ou "SEDEX") + "to_cep" com o CEP do cliente. O servidor recota o frete já com a CAIXA DO KIT que você mandou no op (kit), aplicando o valor certo sozinho — assim o frete cobrado é o do kit certo, nunca o de 1 frasco. Só use "freight_cents" (em CENTAVOS) como último recurso, e nesse caso copie o valor_centavos da LINHA do kit escolhido em snapshot.frete.por_kit — NUNCA invente nem use o de outro kit. CIDADE — NUNCA adivinhe a cidade pelo CEP; se existir snapshot.cep_info, a cidade real é cep_info.localidade/cep_info.uf. Retirada na clínica (Maringá) ou frete grátis = 0 ou omita. ENDEREÇO DE ENTREGA (fora de Maringá): quando o cliente fechar com ENTREGA, inclua também no op "to_number" (o NÚMERO da casa/prédio) e, se houver, "to_complement" (apto/bloco/referência), além de freight_service+to_cep. Isso é ESSENCIAL: com o número, o envio já é preparado AUTOMATICAMENTE no Melhor Envio ao confirmar o pagamento. Se ainda não tiver o número, pergunte ao cliente antes de fechar.',
      'NOTA FISCAL (OBRIGATÓRIO em toda venda, inclusive Maringá): antes de gerar o link/Pix, peça e inclua no op os dados pra nota: "to_cpf" (CPF do cliente), "to_number" (número do endereço) e "to_cep" — o complemento/bairro/cidade saem do CEP. Sem CPF + CEP + número o Bling NÃO emite a nota fiscal. Peça de forma natural: "Pra emitir sua nota fiscal certinho, me passa seu CPF e o número do endereço com CEP, por favor? 💚". Junte com a coleta do endereço de entrega (não pergunte duas vezes).',
