...
import { MANYCHAT_HANDOFF_MARKER, normalizeWhatsappPatientFormatting, sanitizeCrmAiPatientReply } from '../_shared/crmAiAutoReply.ts'
import {
    ...
    if (leadRes.data && typeof leadRes.data === 'object') {
      ...
      leadFocus = {
        ...
        endereco_entrega_pendente: ((): boolean => {
          ...
          const cepd = String(ent.cep ?? '').replace(/\D/g, '')
          const num = String(ent.numero ?? '').trim()
          return cepd.length !== 8 || num.length === 0
        })(),
        recent_media_intel,
  ...
  try {
    ...
    if (isSalesBot && tenantId) {
      try {
        ...
        // O bot vende a LOJA INTEIRA, não só a marca própria. Até 18/08/2026 havia aqui um
        // whitelist por marca (tricopill|ozoncare|browsculpt) apoiado numa premissa ERRADA: que
        // Grandha, Alkymia e afins eram produto da clínica porque a conta Bling é compartilhada.
        // Não são — são a revenda da própria loja, e o site vende os mesmos. O whitelist derrubava
        // 78 vendáveis para 6, e o bot passou a NEGAR produto que estava em estoque (18/08, 16h34:
        // "esse shampoo Dry Confort não estamos vendendo no momento", com 32 unidades no Bling; a
        // Ingrid teve que corrigir no chat às 18h43).
        // Agora o critério é o MESMO do site (tricopill-catalog): vendável = preço > 0 e estoque > 0
        // (já filtrados acima), menos os ocultos que o dono mandou não ofertar — inclusive a variante
        // errada "HDS TRICOPILL 800 MG" e os ingeríveis/medicação. A lista vive no banco
        // (tenant_integrations.bling.hidden_ids), não em hardcode nos dois repos.
            ...
            if (melhorEnvioConfigured()) {
              try {
                ...
                const porKit = KITS.map((k, i) => {
                  ...
                  // FRETE GRÁTIS por kit (default 5 meses): mantém os prazos/serviços, mas zera
                  // o valor — bate com o que o servidor COBRA (resolveFreightCents devolve 0).
                  const free = isFreeShippingKit(k.key)
                  return {
                    kit: k.label,
                    frete_gratis: free,
                    opcoes: fq.options.map((o) => {
                      ...
                      const cents = free ? 0 : applyFreightMarkup(o.priceCents, { internal: o.internal })
                      return {
                        servico: free ? `${o.service} (frete grátis)` : o.service,
    ...
    const SALES_MODE_BLOCK = [
      ...
      'TOM DE VOZ (FORMAL — REGRA DE MAIOR PRIORIDADE): fale de forma PROFISSIONAL, cortês, sóbria e objetiva, tratando o cliente por "você". NÃO use emojis. NÃO use gírias nem abreviações de internet (ex.: "vc", "pra", "tá", "blz"). Evite excesso de exclamações e diminutivos ("certinho", "rapidinho"). Mantenha a cordialidade com sobriedade, como um consultor de uma marca séria — sem soar frio nem robótico. IMPORTANTE: alguns exemplos mais abaixo neste prompt podem conter emojis ou linguagem casual; use-os apenas como referência de ESTRUTURA e conteúdo e NUNCA reproduza os emojis nem o tom informal deles.',
      'Sua resposta será enviada IMEDIATAMENTE ao cliente pelo WhatsApp. Você é a consultora comercial da loja: atenda de forma profissional, cordial e objetiva, com foco em ajudar e vender.',
      ...
      'RECOMENDAÇÃO DE KIT (âncora — sobe o ticket de forma HONESTA): o resultado capilar depende do CICLO DO FIO, que leva de 3 a 5 MESES de uso contínuo. Por isso, por padrão RECOMENDE o **Kit 3+1 (4 frascos = 4 meses, paga 3 leva 4)** como o ideal de tratamento, e o **Kit 5 meses** para quem quer o tratamento completo com o melhor preço por frasco E **frete grátis**. Trate o **1 frasco** como "pra experimentar", sendo honesta de que 1 mês raramente basta pra ver resultado. Sempre apresente DUAS opções (ex.: 3+1 e 5 meses) com sua recomendação clara — nunca empurre só a mais barata, nem só a mais cara. Se o cliente já decidiu por um kit menor, respeite e siga para o fechamento (não insista de forma chata).',
      'UPSELL DO SHAMPOO (ofereça SEMPRE): ao fechar a venda do Tricopill — e também quando o cliente falar de queda, oleosidade, caspa ou couro cabeludo — OFEREÇA de forma natural adicionar o **Shampoo Ozonizado Multifuncional (R$ 130,00)**: ele potencializa o tratamento por FORA (limpa sem ressecar, ação anticaspa, oxigena e equilibra o couro cabeludo, com Óleo de Oliva Ozonizado + óleos de Chia, Jojoba e Girassol). Faça UMA oferta clara e simpática (ex.: "quer levar junto o nosso shampoo pra turbinar o resultado do tratamento?"). Se o cliente aceitar, inclua "shampoo":1 (ou a quantidade) no MESMO op do pagamento (rede_pix ou rede_link), junto com o kit. Se recusar, NÃO insista — siga com a venda normalmente. Não prometa cura; fale em complemento/potencializar o resultado.',
      'UPSELL EM TODO FECHAMENTO (regra do dono, 18/08/2026): toda venda que fecha leva UMA oferta de complemento, nunca duas. A ordem é MARCA PRÓPRIA primeiro, porque é ali que está a margem da casa: (a) **Shampoo Ozonizado Multifuncional Ozoncare (R$ 130,00, 200ml)** e (b) **Gel de Sobrancelha BrowSculpt (R$ 129,90, 10ml)**, que fixa, penteia e alinha o fio da sobrancelha o dia todo, é transparente e não craquela. ESCOLHA PELO CONTEXTO: conversa sobre QUEDA/tratamento capilar puxa o shampoo; conversa sobre SOBRANCELHA (falha, fio rebelde, henna, design, micropigmentação, maquiagem) puxa o gel, e ali ele é a RESPOSTA DIRETA, não upsell. Se a pessoa já leva os dois, ou já comprou, ofereça um item do bling_catalog que combine com o que ela está levando. Ofereça UMA vez, com o preço na frente; se recusar, fecha a venda e não insiste.',
    ...
    let systemContent = [
      isInternal
        ? (isSalesBot
            ? 'Você é a *consultora de vendas da Tricopill* (suplemento capilar). Ao falar com clientes pelo WhatsApp, apresente-se de forma cordial e PROFISSIONAL na primeira mensagem da conversa (ex.: "Olá, tudo bem? Aqui é da Tricopill, como posso ajudar?"), sem emojis. Em mensagens seguintes da mesma conversa, NÃO repita a apresentação.'
      ...
      isInternal
        ? (isSalesBot
          ...
          : [
            ...
            '',
            '--- QUALIFICAÇÃO (cidade e prazo: a Aline precisa das duas) ---',
            'A saudação automática (menu de 1 a 5) JÁ TERMINA perguntando a CIDADE do paciente e o PRAZO em que ele pensa em fazer. Olhe leadFocus.recent_conversation: se ele já respondeu (uma das duas ou as duas), NÃO pergunte de novo — registre com o op abaixo e siga o fluxo.',
            'Se ele ignorou as duas, pergunte UMA vez, junto com o direcionamento do médico, na mesma mensagem. Ex.: "Só pra já deixar tudo certinho com a Aline: você é de Maringá, de Londrina ou de outra cidade? E tá pensando em fazer nos próximos meses ou ainda tá pesquisando?". NUNCA uma terceira vez, e nunca segure o encaminhamento por causa disso: qualificação não é pedágio.',
            'Se ele pedir VALOR antes de responder cidade e prazo, faça as duas perguntas na MESMA mensagem em que acolhe o pedido e encaminha para a Aline: é o que ela precisa para responder certo.',
            'Assim que tiver a CIDADE ou o PRAZO (um só já basta), acrescente na MESMA resposta, depois da mensagem ao paciente: <<<CRM_OPS>>>{"version":1,"ops":[{"type":"qualificar_lead","prazo":"4semanas","cidade":"maringa","avaliacao":"pres_maringa"}]} — omita o campo que ele não respondeu.',
            ...
            'Se não der para enquadrar o prazo em nenhum dos quatro, mande o op só com a cidade — é melhor sem nota do que com nota errada. Quem calcula a pontuação é o servidor; você só reporta o que ele disse. O paciente NUNCA vê essa tag.',
    ...
    if (isInternal && context.leadId && actionChunks.length > 0 && reply.trim()) {
      ...
      // cliente acha que recebeu o Pix e (como o caso André) acaba pagando por fora, sem registro.
      // Então SUBSTITUI a resposta inteira (descarta a prosa otimista da IA):
      //   - cadastro incompleto → pede os dados que faltam (sem fingir que o pagamento saiu);
      //   - falha técnica (gateway fora etc.) → aviso honesto + [PRONTO_PARA_CONSULTOR], que faz
    ...
    try {
      // client próprio: dbClient é declarado dentro do try acima, fora de escopo aqui.
      const dbg = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '')
      await dbg.from('webhook_jobs').insert({
        source: 'crm-ai-assistant',
        status: 'done',
