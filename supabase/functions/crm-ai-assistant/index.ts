  ...
  try {
    ...
    if (isSalesBot && tenantId) {
      try {
        ...
        // O bot do Tricopill vende o Tricopill + a linha Ozoncare + o BrowSculpt (mesma operação,
        // marca própria). A conta Bling é compartilhada com a clínica (Grandha, Alkymia, shampoos,
        // óleos…), então o catálogo do bot é um WHITELIST por marca — e tiramos a variante ERRADA
        ...
        if (tenantId === 'tricopill') {
          sellable = sellable.filter((i) => {
            ...
            return (n.includes('tricopill') || n.includes('ozoncare') || n.includes('browsculpt')) && !n.includes('hds')
          })
        }
        if (sellable.length) {
    ...
    const SALES_MODE_BLOCK = [
      ...
      'GEL DE SOBRANCELHA BROWSCULPT (novidade — ofereça QUANDO CABE, não em toda conversa): temos o **BrowSculpt, gel de sobrancelha com alta fixação (R$ 129,90, 10ml)** — marca PRÓPRIA nossa, igual ao Tricopill. Ele penteia, alinha e segura os fios da sobrancelha o dia todo, é transparente (serve pra qualquer tom), não craquela nem deixa resíduo branco. Ofereça quando: (a) o cliente falar de SOBRANCELHA (falha, fio rebelde, henna, design, micropigmentação, maquiagem) — aí é a resposta direta; ou (b) UMA vez no fechamento da venda, como "leve junto" barato ("quer levar nosso gel de sobrancelha BrowSculpt por R$ 129,90 junto?"). NÃO ofereça o gel no meio de uma conversa sobre queda de cabelo/tratamento capilar — ali o complemento é o shampoo. Se aceitar, mande "gel_sobrancelha":1 (ou a quantidade) no MESMO op do pagamento (rede_pix ou rede_link), junto com o kit ou sozinho. Se recusar, não insista.',
      ...
      'KIT no op: passe "kit" com a chave — "1_mes", "3_meses" ou "5_meses" — quando vender um kit. O servidor aplica o PREÇO do kit sozinho; você NÃO calcula nem manda o valor — só o frete (freight_service+to_cep). ⚠️ PROIBIDO vender por valor avulso: NUNCA use "amount_cents"/"description". Além dos kits existem DOIS produtos avulsos: o **Shampoo Ozonizado Multifuncional (R$ 130,00 a unidade)** → "shampoo":N no op; e o **Gel de Sobrancelha BrowSculpt (R$ 129,90 a unidade)** → "gel_sobrancelha":N no op. Qualquer um dos dois pode ir JUNTO com o kit (upsell) OU sozinho, e podem ir os dois juntos. O servidor soma os valores e cria o pedido no Bling com cada item real (baixando o estoque). Fora isso (kits + esses dois), não vendemos nada; sem "kit" E sem nenhum avulso o servidor RECUSA o pagamento. Se o cliente pedir outro produto, ofereça o mais próximo ou encaminhe pra atendente.',
