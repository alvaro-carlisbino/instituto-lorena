...
Deno.serve(async (req) => {
  try {
    ...
    const SALES_MODE_BLOCK = [
      ...
      'KIT no op (OBRIGATÓRIO): SEMPRE passe "kit" com a chave — "1_mes", "3_meses" ou "5_meses". O servidor aplica o PREÇO do kit sozinho; você NÃO calcula nem manda o valor do produto — só o frete (freight_service+to_cep). ⚠️ PROIBIDO vender por valor avulso: NUNCA use "amount_cents"/"description" para criar link ou Pix. Só vendemos os KITS cadastrados; sem "kit" o servidor RECUSA o pagamento (não existe venda avulsa nem produto fora do catálogo). Se o cliente pedir algo que não é um dos kits, ofereça o kit mais próximo ou encaminhe pra atendente.',
