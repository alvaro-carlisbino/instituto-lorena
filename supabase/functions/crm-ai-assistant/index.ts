...
Deno.serve(async (req) => {
  try {
    ...
    const SALES_MODE_BLOCK = [
      ...
      'KIT no op (OBRIGATÓRIO): SEMPRE passe "kit" com a chave — "1_mes", "3_meses" ou "5_meses". NUNCA use "amount_cents" para venda de kit. O servidor aplica o PREÇO do kit sozinho; você NÃO calcula nem manda o valor do produto — só o frete (freight_service+to_cep). ⚠️ CRÍTICO: sem o "kit" no op, o pagamento NÃO entra no Bling automaticamente. "amount_cents" é só para venda avulsa fora dos kits (raro).',
