...
async function sendWapiText(admin: SupabaseClient, phone: string, text: string): Promise<boolean> {
  ...
  try {
    const { data } = await admin.from('whatsapp_channel_instances').select('wapi_instance_id, wapi_token, wapi_base_url').eq('tenant_id', TENANT_ID).eq('channel_provider', 'wapi').eq('active', true).order('sort_order', { ascending: true }).order('created_at', { ascending: true }).limit(1).maybeSingle()
